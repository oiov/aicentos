...
- 已获取 NBility API Key（[控制台获取](https://nbility.dev/console/token)）
...
::: warning 重要
- 请将 `sk-你的NBility-Token` 替换为你在 [NBility 控制台](https://nbility.dev/console/token) 获取的实际 Token
- **Anthropic 协议的 `baseUrl` 不要带 `/v1`**，SDK 会自动拼接路径
:::

#### 配置 OpenAI（GPT）模型

通过 NBility 调用 OpenAI 模型时，`api` 字段须设为 `openai-responses`：
...
::: tip 提示
**OpenAI 协议需要带 `/v1`**，即 `https://nbility.dev/v1`。这是因为两种 SDK 的路径拼接逻辑不同。
:::
...
| --- | --- | --- | --- |
| `baseUrl` | API 代理地址 | `https://nbility.dev` | `https://nbility.dev/v1` |
| `apiKey` | 你的 API Key | `sk-你的NBility-Token` | `sk-你的NBility-Token` |
...
如果配置正确，OpenClaw 将正常启动并通过 NBility 代理发送请求。
...
**解决**：通过 NBility 使用时，Claude 模型填 `anthropic-messages`，GPT 模型填 `openai-responses`。
...
**解决**：通过 NBility 调用 GPT 模型时，请使用 `openai-responses` 而非 `openai-completions`。
...
- **openai-responses** — 对应 OpenAI Responses API（`/v1/responses`）。**通过 NBility 使用 GPT 模型时用这个。**
- **openai-completions** — 对应 OpenAI Chat Completions API（`/v1/chat/completions`），通用兼容格式，但在 NBility 的 GPT 模型上可能导致空回复，**不推荐使用**。
...
::: tip 排查核心思路
先用 curl 确认 NBility API 本身正常，再检查 OpenClaw 端发出的请求有什么不同（UA、路径、格式）。
:::
