...
NBility 仅作为 API 中转服务，不存储你的代码和对话内容。所有请求直接转发至对应的模型提供商。
...
访问 [nbility.dev/register](https://nbility.dev/register?aff=Dptp)，按提示完成注册即可。

### 如何获取 API Token？

注册后登录控制台，前往 [Token 管理页](https://nbility.dev/console/token) 创建新的 Token。
...
NBility 为每个用户提供用量额度，具体数量根据平台资源动态调整。请关注控制台中的用量信息。
...
1. 确认 `BASE_URL` 设置为 `https://nbility.dev/`（注意末尾的 `/`）
2. 检查网络是否能访问 `nbility.dev`
...
解决方法：前往 [控制台](https://nbility.dev/console/token) 确认 Token 状态，重新复制粘贴。
...
检查指定的模型名称是否正确，参考 [快速开始](/start) 中的推荐模型列表。部分模型可能尚未在 NBility 上线。
